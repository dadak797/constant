...
void App::initLogger(bool useConsole, bool newFile) {
  if (useConsole) {
    ...
    auto file_logger = spdlog::basic_logger_mt(
        "file_logger", "logs/vtk-workbench.log", newFile);
...
void App::setStyle(AppStyle style) {
  m_Style = style;
  ...
  EM_ASM(
      ...
      APP_STYLE_KEY, static_cast<int>(m_Style));
#endif
}
...
void App::SetInitStyle() {
  ...
  AppStyle initStyle = static_cast<AppStyle>(viewerStyle);
  setStyle(initStyle);
#endif
}
...
void App::renderDockSpaceAndMenu() {
  ...
  if (m_bFullDockSpace) ImGui::PopStyleVar(2);
  ...
  if (ImGui::BeginMenuBar()) {
    ...
    if (ImGui::BeginMenu("Settings")) {
      if (ImGui::BeginMenu("Style")) {
        if (ImGui::MenuItem("Dark", nullptr, m_Style == AppStyle::DARK)) {
          SetStyle(AppStyle::DARK);
        }
        if (ImGui::MenuItem("Light", nullptr, m_Style == AppStyle::LIGHT)) {
          SetStyle(AppStyle::LIGHT);
        }
        if (ImGui::MenuItem("Classic", nullptr, m_Style == AppStyle::CLASSIC)) {
          SetStyle(AppStyle::CLASSIC);
        }
        ImGui::EndMenu();
      }
