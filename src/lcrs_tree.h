...
class LcrsTree {
 public:
  ...
  // If the id already exists or the label is empty, the item will not be
  // inserted and return nullptr.
  TreeNode* InsertItem(int32_t id, const char* label,
                       TreeNode* parent = nullptr);
  bool DeleteItem(int32_t id);

  // Traverse all tree nodes
  // The callback function will be called for each node.
  // The userData parameter can be used to pass additional data to the callback
  // function.
 ...
 private:
  ...
  void traverseTreeRecursiveMutable(
